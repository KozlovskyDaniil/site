		...
		private function pageOverview($return_array = false){
			...
			foreach($files as $filename => $regions){
				...
				foreach($regions as $i => $region){
					echo '<a rel="tooltip" data-html="true" href="./index.php?page=page_edit&amp;filename=' . urlencode($current_directory . $filename) . '&amp;region=' . $region . '" title="' . htmlspecialchars($this->getLanguage('edit_region',$region)) . '">' . $region . '</a>&nbsp;&nbsp;<i class="fa fa-pencil" title="' . htmlspecialchars($this->getLanguage('edit_page')) . '"></i>&nbsp;&nbsp;';
				}
				echo  '		</td>';
		...
		private function pageEdit($filename, $region){
					...
					if(preg_match_all('#<meta([^<]+)>#ism',$content,$matches)){
						foreach($matches[1] as $i => $match){
							if(preg_match('#name\s*=\s*("|\'){1}keywords("|\'){1}#ism',$match,$matches_n) && preg_match('#(content\s*=\s*("|\'))([^"\']*)("|\')#ism',$match,$matches_v)){
								$meta = str_replace($matches_v[0],$matches_v[1] . $_POST['keywords'] . $matches_v[4],$matches[0][$i]);
					...
					if(preg_match_all('#<meta([^<]+)>#ism',$content,$matches)){
						foreach($matches[1] as $i => $match){
							if(preg_match('#name\s*=\s*("|\'){1}description("|\'){1}#ism',$match,$matches_n) && preg_match('#(content\s*=\s*("|\'))([^"\']*)("|\')#ism',$match,$matches_v)){
								$meta = str_replace($matches_v[0],$matches_v[1] . $_POST['description'] . $matches_v[4],$matches[0][$i]);
								$content = str_replace($matches[0][$i],$meta,$content);
							}
						}
					}
			...
			<script type="text/javascript">
				tinymce.init({
					...
					// Toolbar buttons
					toolbar1: "searchreplace undo redo | paste | bullist numlist outdent indent | ' . ($this->administrator || $this->user && USER_FONT_COLOR == '1' ? 'forecolor backcolor |' : '') . ' charmap hr table | fullscreen code | restoredraft",
